...
/// await video.SaveContactSheetAsync("out.jpg", new ContactSheetOptions { Columns = 3, Rows = 3 });
/// </example>
public sealed class Video
{
    ...
    public async Task<SKBitmap> CaptureFrameAsync(
        TimeIndex time, int width, bool evadeBlank = false,
        double blankThreshold = 0.08, IReadOnlyList<int>? alternatives = null,
        CancellationToken ct = default)
    {
        ...
        foreach (var offset in alternatives ?? new[] { -5, 5, -10, 10, -30, 30 })
        {
            ...
            if (alt.TotalSeconds < 0 || alt.Value > info.Duration) continue;
        ...
        return bitmap; // give up gracefully; keep the original
    }
    ...
    public async Task<byte[]> BuildContactSheetAsync(
        ContactSheetOptions options, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        var info = await GetInfoAsync(ct).ConfigureAwait(false);
        ...
        {
            ct.ThrowIfCancellationRequested();
            var bmp = await CaptureFrameAsync(h, options.ThumbnailWidth, options.BlankEvasion,
                options.BlankThreshold, options.BlankAlternatives, ct).ConfigureAwait(false);
        ...
        {
            ct.ThrowIfCancellationRequested();
            var bmp = await CaptureFrameAsync(t, options.ThumbnailWidth, options.BlankEvasion,
                options.BlankThreshold, options.BlankAlternatives, ct).ConfigureAwait(false);
    ...
    {
        if (!options.ShowHeader) return Array.Empty<string>();
        ...
        var lines = new List<string>
        {
            ...
            $"Duration: {FormatDuration(info.Duration)}" +
                (v != null ? $"   Resolution: {v.Width}x{v.Height}" : "") +
                (v?.FrameRate is { } fps ? $"   {fps:0.##} fps" : ""),
        };
        var codecLine = "";
        if (v?.Codec != null) codecLine += $"Video: {v.Codec}";
        if (a?.Codec != null) codecLine += (codecLine.Length > 0 ? "   " : "") + $"Audio: {a.Codec}" +
            (a.SampleRate is { } sr ? $" {sr} Hz" : "") +
            (a.Channels > 0 ? $" {a.Channels}ch" : "");
        if (codecLine.Length > 0) lines.Add(codecLine);

        return lines.ToArray();
    }

    private static string FormatDuration(TimeSpan d)
        => d.TotalHours >= 1 ? $"{(int)d.TotalHours}:{d.Minutes:D2}:{d.Seconds:D2}" : $"{d.Minutes:D2}:{d.Seconds:D2}";
